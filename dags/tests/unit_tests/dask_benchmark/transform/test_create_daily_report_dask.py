...
from functions.dask_benchmark.functions.transform import create_daily_report
...
def test_empty_frame() -> None:
  df = pd.DataFrame({
    "pickup_date": pd.Series(dtype="string"),
    "pickup_day": pd.Series(dtype="string"),
    "PULocationID": pd.Series(dtype="string"),
    "DOLocationID": pd.Series(dtype="string"),
    "trip_distance": pd.Series(dtype="float64"),
    "fare_amount": pd.Series(dtype="float64"),
    "tip_amount": pd.Series(dtype="float64"),
    "tip_ratio": pd.Series(dtype="float64"),
    "passenger_count": pd.Series(dtype="int64"),
    "VendorID": pd.Series(dtype="int64"),
    "avg_speed": pd.Series(dtype="float64"),
  })
  sample_data = dd.from_pandas(df, npartitions=1)
  locations = pd.DataFrame([], columns=location_map_columns)
  sample_locations = dd.from_pandas(locations, npartitions=1)
  result_data = create_daily_report(sample_data, sample_locations).compute()
